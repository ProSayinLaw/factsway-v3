...
4. [Case Block Specification](#4-case-block-specification)
5. [Signature Block Specification](#5-signature-block-specification)
6. [Body Editor Specification](#6-body-editor-specification)
7. [Section Hierarchy System](#7-section-hierarchy-system)
8. [Footer & Page Numbering](#8-footer--page-numbering)
9. [Document Parsing Rules](#9-document-parsing-rules)
...
14. [Preview System](#14-preview-system)
15. [Technology Stack](#15-technology-stack)
16. [File Structure](#16-file-structure)
17. [Pre-built Texas Template](#17-pre-built-texas-template)
18. [Verification Checklist](#18-verification-checklist)
19. [Design System](#19-design-system)
    - [19.1 Design Tokens](#191-design-tokens)
    - [19.2 Typography](#192-typography)
    - [19.3 Color Semantics](#193-color-semantics)
    - [19.4 Component Specifications](#194-component-specifications)
    - [19.5 Layout Patterns](#195-layout-patterns)
    - [19.6 Interaction Patterns](#196-interaction-patterns)
    - [19.7 View Specifications](#197-view-specifications)
20. [Execution Tracing](#20-execution-tracing)
    - [20.1 Design Principles](#201-design-principles)
    - [20.2 Pipeline Event Schema](#202-pipeline-event-schema)
    - [20.3 Invariant Result Schema](#203-invariant-result-schema)
    - [20.4 Pipeline Boundaries](#204-pipeline-boundaries)
    - [20.5 Trace Storage](#205-trace-storage)
    - [20.6 Invariant Definitions by Domain](#206-invariant-definitions-by-domain)
    - [20.7 Failure Handling](#207-failure-handling)
    - [20.8 Trace Query Interface](#208-trace-query-interface)
    - [20.9 UI Integration](#209-ui-integration)
    - [20.10 Implementation Notes](#2010-implementation-notes)
...
│                         BROWSER (Frontend)                          │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌────────────┐ │
│  │  Template   │  │    Case     │  │   Motion    │  │   Export   │ │
│  │   Builder   │  │   Manager   │  │   Editor    │  │  Controls  │ │
│  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘  └─────┬──────┘ │
│         │                │                │                │        │
│         └────────────────┴────────────────┴────────────────┘        │
│                                   │                                  │
...
│                          BACKEND (FastAPI)                            │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌───────────┐ │
│  │   /ingest    │  │   /export    │  │  /preview    │  │  /health  │ │
│  │  (parse doc) │  │ (build docx) │  │ (docx→pdf)   │  │           │ │
│  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘  └───────────┘ │
│         │                 │                 │                         │
│         ▼                 ▼                 ▼                         │
│  ┌─────────────────────────────────────────────────────────────────┐ │
│  │                      Document Engine                             │ │
│  │  ┌─────────┐  ┌─────────────┐  ┌────────────┐  ┌──────────────┐ │ │
│  │  │ Parser  │  │   Pandoc    │  │  OOXML     │  │ LibreOffice  │ │ │
│  │  │ + LLM   │  │  Converter  │  │  Injector  │  │  Converter   │ │ │
│  │  └─────────┘  └─────────────┘  └────────────┘  └──────────────┘ │ │
│  └─────────────────────────────────────────────────────────────────┘ │
└───────────────────────────────────────────────────────────────────────┘
```
...
This project is built through 15 discrete runbooks, each self-contained and executable with context reset between them.
...
| 2 | OOXML Generators | case_block_generator.py, footer_generator.py | 0 |
| 3 | Pandoc Integration | pandoc_wrapper.py, footnote_transformer.py | 0 |
| 4 | Document Merger | document_merger.py, signature_injector.py | 0, 2, 3 |
| 5 | Sentence Parser | sentence_parser.py (NUPunkt + LLM verification) | 0 |
| 6 | Citation Resolver | citation_resolver.py, evidence_map.py | 0, 5 |
| 7 | Variable Resolver | variable_resolver.py | 0 |
| 8 | Export Pipeline | pipeline.py (orchestrator) | 0, 2-7 |
| 9 | Document Parser | All parser/ modules | 0 |
| 10 | API Layer | All api/ modules | 0, 8, 9 |
| 11 | Frontend - Core | index.html, useStorage, useEditor, base components | 0 |
| 12 | Frontend - Template Builder | TemplateBuilder.js, variable editor | 0, 11 |
| 13 | Frontend - Editor | DraftEditor.js, Toolbar.js, citation detection | 0, 11 |
| 14 | Frontend - Evidence System | Evidence sidebar, Evidence map, linking modals | 0, 11, 13 |
| 15 | Container & Tests | Dockerfile, docker-compose.yml, all tests | 0-14 |

**New Runbooks Explained:**

| Runbook | Purpose |
|---------|---------|
| 5 - Sentence Parser | Implements NUPunkt sentence detection with citation-aware splitting and surgical LLM verification for edge cases. Produces stable sentence IDs for evidence linking. |
| 6 - Citation Resolver | Resolves CitationNodes to display text. Computes exhibit markers. Builds evidence map data structure. |
| 7 - Variable Resolver | Resolves template variables including computed values (date offsets, conditionals). |
| 14 - Evidence System | The interactive sidebar, drag-drop, document viewer, and evidence map panel. |
...
- Prevents drift from accumulated context errors
- Each runbook is small enough to execute reliably
- Failed runbook can be re-run without affecting others
- Clear dependency chain for debugging
- Evidence system is isolated in its own runbook (14) to manage complexity
...
### 2.6 Citation Node Model

Citations are inline nodes that reference uploaded evidence (exhibits, case law opinions, statutes) and optionally link to the sentences they support.
...
// Result: Text preserved, wrapped in citation node
...
## 5. Signature Block Specification
...
editor.commands.insertContent({
  ...
  attrs: {
    evidence_id: 'EV-001',
    citation_text: '[1]',
    sentence_ids: ['s-a3f2b1c4-d5e6-4f7a-8b9c-0d1e2f3a4b5c-001'],
  },
});
```
...
**System Binaries:**
```yaml
pandoc: "3.6+"                # MINIMUM 3.6 - Required for HTML footnote parsing with dpub-aria roles
libreoffice: "7.6.4"          # For PDF preview
```
...
python-docx==1.1.0
lxml==5.1.0
pandoc==2.3              # Python wrapper for Pandoc CLI

# Sentence Parsing & Citation Detection
nupunkt==0.1.3           # Legal-focused sentence boundary detection
eyecite==2.6.2           # Case law citation extraction
openai==1.10.0           # For LLM verification
...
# 5. Install system dependencies (see below)
```
...
Pandoc must be version 3.6.0 or higher for HTML footnote support.
...
# Install pandoc-3.6.1-windows-x86_64.msi

# Verify installation
pandoc --version  # Should show 3.6.x
```

**LibreOffice (required for PDF preview)**

LibreOffice must be version 7.6+ for headless conversion reliability.
...
# Verify installation
libreoffice --version  # Should show 7.6.x
```

**OpenAI API Key (required for LLM verification)**
...
python -c "import nupunkt; print('NUPunkt installed')"
python -c "import eyecite; print(f'eyecite {eyecite.__version__}')"
...
| Issue | Solution |
|-------|----------|
| `nupunkt` not found on PyPI | Install from GitHub: `pip install git+https://github.com/pmbaumgartner/nupunkt.git@v0.1.3` |
| `eyecite` ImportError | Ensure lxml is installed first: `pip install lxml==5.1.0` |
| Pandoc version too old | Uninstall system Pandoc, install from GitHub releases |
| LibreOffice headless fails | Install full LibreOffice (not just core), verify with `libreoffice --headless --version` |
| OPENAI_API_KEY not found | Set in .env file or export in shell before running server |

**Docker Alternative:**

If system dependency installation is problematic, use the provided Dockerfile (Runbook 15) which includes all dependencies in a containerized environment.

**Frontend (via CDN):**
```yaml
vue: "3.4.15"
tiptap_core: "2.1.13"
tiptap_starter_kit: "2.1.13"
tiptap_extension_table: "2.1.13"
tiptap_extension_image: "2.1.13"
tiptap_extension_underline: "2.1.13"
tailwindcss: "3.4.1"
```

### 15.6 CDN URLs (Verified)
...
<script type="importmap">
{
  "imports": {
    ...
    "@tiptap/vue-3": "https://esm.sh/@tiptap/vue-3@2.1.13",
...
|--------|----------------|---------------|
| `pipeline.py` | Orchestrates complete export flow | `export_document()`, `preview_document()` |
| `case_block_generator.py` | Generates case block with § alignment | `generate_case_block_ooxml()`, `build_party_rows()` |
| `footer_generator.py` | Generates three-column footers | `add_three_column_footer()`, `create_page_number_cell()` |
| `pandoc_wrapper.py` | Pandoc CLI invocation | `convert_html_to_docx()`, `get_pandoc_version()` |
| `footnote_transformer.py` | Converts Tiptap footnotes to Pandoc format | `transform_footnotes_for_pandoc()` |
| `document_merger.py` | Merges Pandoc output into main document | `merge_body_into_document()`, `copy_footnotes()` |
| `signature_injector.py` | Injects signature/certificate OOXML | `inject_signature()`, `inject_certificate()`, `populate_variables()` |
| `image_processor.py` | Post-processes images for print | `constrain_image_widths()` |
| `sentence_parser.py` | NUPunkt detection + surgical LLM verification | `compute_sentence_registry()`, `parse_sentences()`, `verify_boundary_with_llm()`, `verify_low_confidence_boundaries()` |
| `citation_resolver.py` | Resolves citations to display text | `resolve_citations()`, `number_to_alpha()`, `prosemirror_to_html()` |
| `evidence_map.py` | Computes evidence map data | `compute_evidence_map()`, `looksLikeClaim()` |
| `variable_resolver.py` | Resolves template variables | `resolve_variables()`, `compute_variable()`, `topological_sort_variables()` |
| `pdf_converter.py` | LibreOffice PDF conversion | `convert_to_pdf()` |
| `extractor.py` | Main parsing orchestrator | `parse_document()`, `extract_components()` |
| `patterns.py` | Regex patterns | `COURT_PATTERN`, `CAUSE_NUMBER_PATTERN`, etc. |
| `structure_parser.py` | Section hierarchy extraction | `parse_sections()`, `detect_heading_level()` |
| `ooxml_extractor.py` | Raw OOXML extraction | `extract_signature_ooxml()`, `extract_region()` |
| `llm_detector.py` | AI body start detection | `detect_body_start()` |
...
│
├── reference.docx                    # Style template for Pandoc
│
├── engine/
│   ├── __init__.py
│   ├── pipeline.py                   # Main export orchestrator
│   ├── case_block_generator.py       # Case block OOXML generation
│   ├── footer_generator.py           # Three-column footer OOXML
│   ├── pandoc_wrapper.py             # Pandoc CLI invocation
│   ├── footnote_transformer.py       # tiptap-footnotes → Pandoc HTML
│   ├── document_merger.py            # Merge Pandoc output into main doc
│   ├── signature_injector.py         # Signature/certificate OOXML injection
│   ├── image_processor.py            # Image width constraints
│   ├── sentence_parser.py            # NUPunkt + LLM sentence detection
│   ├── citation_resolver.py          # Citation → display text resolution
│   ├── evidence_map.py               # Evidence map data computation
│   ├── variable_resolver.py          # Template variable resolution
│   └── pdf_converter.py              # LibreOffice integration
│
├── parser/
│   ├── __init__.py
│   ├── extractor.py                  # Main extraction orchestrator
│   ├── patterns.py                   # Regex patterns for deterministic extraction
│   ├── structure_parser.py           # Section hierarchy parser
│   ├── ooxml_extractor.py            # Extract OOXML regions from docx
│   └── llm_detector.py               # OpenAI integration for body start
│
├── api/
│   ├── __init__.py
│   ├── main.py                       # FastAPI app initialization
│   ├── routes.py                     # Endpoint handlers
│   └── schemas.py                    # Pydantic models (from this spec)
│
├── static/
...
│   │   ├── TemplateBuilder.js        # Template creation/editing
│   │   ├── VariableEditor.js         # Template variable configuration
│   │   ├── CaseManager.js            # Case list and creation
│   │   ├── DraftEditor.js            # Main editor with Tiptap
│   │   ├── Toolbar.js                # Formatting toolbar
│   │   ├── EvidenceSidebar.js        # Evidence panel with drag-drop
│   │   ├── EvidenceMap.js            # Claim-evidence visualization
│   │   ├── DocumentViewer.js         # PDF/image viewer modal
│   │   ├── CitationNode.js           # Tiptap node view for citations
│   │   ├── CrossReferenceNode.js     # Tiptap node view for cross-refs
│   │   ├── CitationLinkingModal.js   # Link citation to evidence
│   │   ├── SentenceLinkingModal.js   # Link citation to sentences
│   │   ├── VariablesPanel.js         # Draft variable values input
│   │   └── ExportPreview.js          # Preview and export controls
│   │
│   ├── composables/
│   │   ├── useStorage.js             # localStorage abstraction
│   │   ├── useEditor.js              # Tiptap editor management
│   │   ├── useCitationIndex.js       # Citation marker computation
│   │   ├── useEvidenceMap.js         # Evidence map computation
│   │   ├── useSentenceRegistry.js    # Sentence registry access
│   │   └── useVariables.js           # Variable resolution
│   │
│   ├── extensions/
│   │   ├── Citation.js               # Tiptap citation node extension
│   │   ├── CrossReference.js         # Tiptap cross-reference extension
│   │   ├── Variable.js                # Tiptap variable node extension
│   │   ├── CitationDetection.js      # Pattern detection decoration
│   │   ├── EvidenceDropTarget.js     # Drag-drop handler
│   │   └── SentenceHighlight.js      # Sentence highlighting on hover
│   │
│   ├── styles/
│   │   ├── main.css                  # Global styles
│   │   ├── editor.css                # Tiptap editor styles
│   │   ├── evidence.css              # Evidence sidebar/map styles
│   │   └── modals.css                # Modal dialog styles
│   │
│   └── assets/
│       └── default_template.json     # Pre-built Texas template
│
└── tests/
    ├── fixtures/
    │   ├── sample_motion.docx        # Test input document
    │   ├── sample_signature.docx     # Test signature block
    │   ├── sample_opinion.pdf        # Test case law upload
    │   └── expected_output.json      # Expected parse result
    ├── test_parser.py
...
### 18.1 Runbook 1 (Reference Document) Verification
...
### 18.2 Runbook 2 Verification

**Runbook 2: OOXML Generators**

- [ ] `python -m pytest tests/test_case_block_generator.py` passes
- [ ] `python -m pytest tests/test_footer_generator.py` passes
- [ ] Output uses correct styles from reference.docx
- [ ] Section numbering matches outline schema
- [ ] Headers render centered/caps/bold correctly

### 18.3 Runbook 3 Verification

**Runbook 3: Pandoc Integration**

Verify Pandoc wrapper correctly converts HTML to DOCX with footnote transformation.

- [ ] `python -m pytest tests/test_pandoc_wrapper.py` passes
- [ ] `python -m pytest tests/test_footnote_transformer.py` passes
- [ ] `python scripts/test_footnote_roundtrip.py` preserves footnotes correctly

### 18.4 Runbook 4 (API Layer) Verification

- [ ] `GET /health` returns 200 with versions
- [ ] `POST /ingest` accepts .docx and returns ParsedDocument
- [ ] `POST /export` returns valid .docx
- [ ] `POST /preview` returns valid PDF
- [ ] Error responses match documented format

### 18.5 Runbook 5 (Editing Surface) Verification

- [ ] App loads without JavaScript errors
- [ ] Template builder saves to localStorage
- [ ] Case data editor saves to localStorage
- [ ] Tiptap editor initializes with formatting toolbar
- [ ] Bold/italic/underline work correctly
- [ ] Heading levels apply correctly
- [ ] Block quotes indent correctly
- [ ] Tables insert and render correctly
- [ ] Section promote/demote works (Tab/Shift+Tab)
- [ ] Enter key creates appropriate next element
- [ ] Paste strips unwanted formatting
- [ ] Case block displays locked state
- [ ] Case edit modal opens and saves changes
- [ ] Signature block displays locked state
- [ ] Page break guides visible at correct intervals
- [ ] Export button triggers download
- [ ] Preview button shows PDF

### 18.6 Runbook 6 (Container) Verification

- [ ] `docker-compose up --build` completes without errors
- [ ] Container starts and shows "Uvicorn running"
- [ ] `curl localhost:8000/health` returns healthy
- [ ] Browser can access application
- [ ] Full upload → edit → export cycle works in container
- [ ] Preview PDF generation works in container

### 18.7 End-to-End Verification

- [ ] Create new template from scratch
- [ ] Create case from template
- [ ] Create draft in case
- [ ] Type motion body with sections
- [ ] Export .docx
- [ ] Open in Microsoft Word
- [ ] Verify: fonts correct
- [ ] Verify: spacing correct
- [ ] Verify: margins correct
- [ ] Verify: section numbers correct
- [ ] Verify: signature block renders correctly
- [ ] Verify: footer shows motion title and page numbers
- [ ] Upload different .docx
- [ ] Verify extraction correct
- [ ] Apply template to extracted content
