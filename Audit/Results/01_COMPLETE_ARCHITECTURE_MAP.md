...
1. [Part 1: Current State](#part-1-current-state)
   ...
   - [IPC Channel Inventory](#ipc-channel-inventory)
...
- **API Routes:** ~12
- **IPC Handlers:** Multiple in src/main/handlers/
...
- Multiple TypeScript route files

**Handlers:** src/main/handlers/
- Multiple IPC handler files
...
|------|-----------|---------|------|
| `src/main/index.ts` | Electron Main | App entry point | 🟢 KEEP |
| `src/main/preload.ts` | Electron Preload | IPC bridge | 🟢 KEEP |
| `src/main/handlers/` | IPC Handlers | Channel handlers | 🟢 KEEP |

### API Routes

| Path | Route | Method | Fate |
|------|-------|--------|------|
| `src/api/routes/*.ts` | Various API endpoints | Multiple | 🟢 KEEP (update to call services) |

### Python Ingestion Pipeline

| Path | Component | Purpose | Fate |
|------|-----------|---------|------|
| `factsway-ingestion/ingestion_engine/` | Python Pipeline | DOCX ingestion (OLD) | 🔴 REFACTOR into ingestion-service |
| `factsway-ingestion/app.py` | Python API | FastAPI server (OLD) | 🔴 REMOVE |
| `factsway-ingestion/ingestion_engine/services/` | Python Services | NLP/Citation services | 🟡 REFACTOR - Extract for new microservices |

### Database & Storage

| Path | Component | Purpose | Fate |
|------|-----------|---------|------|
| `migrations/` | Database Schema | Migration files | 🟢 KEEP |
| `vault/` | Document Storage | Data persistence | 🟢 KEEP |
| `src/main/services/` | Storage Services | Data persistence | 🟢 KEEP |
...
Multiple IPC handlers found in `src/main/handlers/` - all marked 🟢 KEEP

**Status:** All existing IPC channels must remain functional
**Warning:** 118 invocations found vs fewer handlers - verify all channels are properly registered
...
Approximately 12 API routes found in `src/api/routes/`

**Migration Strategy:**
- 🟢 KEEP all route files
- 🟡 UPDATE to call microservices instead of direct DB/Python
- Example: `POST /api/cases/:id/filing/export` will call `export-service:3003`
...
TypeScript/Node dependencies present for:
- Electron framework
- Express API server
...
**Migration Note:** Python dependencies will be distributed across new microservices
...
**Based on Runbook 0, Section 16 (File Structure)**
...
**Desktop Orchestrator:**
- ✨ `apps/desktop/src/main/orchestrator.ts` - Spawns/manages services
...
**Usage:** Reference constantly during implementation
