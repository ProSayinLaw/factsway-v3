...
**Build & Startup:**
- [ ] `npm install` completes without errors in `services/citation-service/`
- [ ] `npm run build` compiles TypeScript successfully
- [ ] `npm run dev` starts service on port 3004
- [ ] Service logs startup message: "✓ Citation Service running on port 3004"
- [ ] No TypeScript compilation errors
- [ ] No runtime errors in console

**Health Check:**
- [ ] `curl http://localhost:3004/health` returns 200
- [ ] Response contains `{ "status": "healthy", "service": "citation-service", "port": 3004 }`
- [ ] Health check responds within 100ms

**Citation Parsing API:**
- [ ] `POST /api/citations/parse` accepts citation string
- [ ] Parses case citations: "123 F.3d 456 (9th Cir. 2020)" correctly
- [ ] Extracts volume (123), reporter (F.3d), page (456), court (9th Cir.), year (2020)
- [ ] Parses statute citations: "42 U.S.C. § 1983" correctly
- [ ] Extracts title (42), section (1983)
- [ ] Identifies "Id." citations as type "id"
- [ ] Identifies "supra" citations as type "supra"
- [ ] Returns `isValid: true` for valid citations
- [ ] Returns `isValid: false` for invalid citations

**Citation Validation API:**
- [ ] `POST /api/citations/validate` accepts citation string
- [ ] Returns validation result with `isValid`, `type`, and `errors` fields
- [ ] Validates case citation format correctly
- [ ] Validates statute citation format correctly
- [ ] Returns appropriate error messages for invalid citations

**Error Handling:**
- [ ] Returns 400 for missing citation parameter
...
**Build & Startup:**
- [ ] `npm install` completes without errors in `services/evidence-service/`
- [ ] `npm run build` compiles TypeScript successfully
- [ ] `npm run dev` starts service on port 3005
- [ ] Service logs startup message: "✓ Evidence Service running on port 3005"
- [ ] Storage directory created: `services/evidence-service/storage/uploads`

**Health Check:**
- [ ] `curl http://localhost:3005/health` returns 200
- [ ] Response contains `{ "status": "healthy", "service": "evidence-service", "port": 3005 }`

**File Upload API:**
- [ ] `POST /api/evidence` accepts multipart/form-data
- [ ] Uploads PDF file successfully
- [ ] Uploads image file (PNG, JPG) successfully
- [ ] Uploads DOCX file successfully
- [ ] Returns evidence record with ID, caseId, type, title, filePath, fileSize, createdAt
- [ ] File saved to storage directory with UUID prefix
- [ ] Returns 400 if no file uploaded
- [ ] Returns 201 on successful upload

**File Retrieval API:**
- [ ] `GET /api/evidence/:id` retrieves file by ID
- [ ] Returns file buffer for existing evidence
- [ ] Returns 404 for non-existent evidence
- [ ] File content matches uploaded content (byte-for-byte)

**File Deletion API:**
- [ ] `DELETE /api/evidence/:id` deletes file by ID
- [ ] File removed from storage directory
- [ ] Returns success message on deletion
- [ ] Returns 404 for non-existent evidence
- [ ] Deletion is permanent (file cannot be retrieved after)
...
**Build & Startup:**
- [ ] `npm install` completes without errors in `services/template-service/`
- [ ] `npm run build` compiles TypeScript successfully
- [ ] `npm run dev` starts service on port 3006
- [ ] Service logs startup message: "✓ Template Service running on port 3006"

**Health Check:**
- [ ] `curl http://localhost:3006/health` returns 200
- [ ] Response contains `{ "status": "healthy", "service": "template-service", "port": 3006 }`

**Template Filling:**
- [ ] Replaces `{{plaintiff}}` with provided value
- [ ] Replaces `{{defendant}}` with provided value
- [ ] Replaces `{{case_number}}` with provided value
- [ ] Replaces all occurrences of variable (not just first)
- [ ] Handles multiple variables in same text
- [ ] Preserves LegalDocument structure after replacement
- [ ] Does not modify document structure (sections, paragraphs, sentences)

**Variable Extraction:**
- [ ] Extracts all variable names from template
- [ ] Returns unique variable names only (no duplicates)
- [ ] Handles variables in section titles
- [ ] Handles variables in paragraph text
- [ ] Handles variables in sentence text
- [ ] Variable names match pattern `{{variableName}}`

**LegalDocument Type Safety:**
- [ ] Input template is valid LegalDocument type
- [ ] Output filled document is valid LegalDocument type
...
**Build & Startup:**
- [ ] `pip install -r requirements.txt` completes in `services/analysis-service/`
- [ ] `python main.py` starts service on port 3007
- [ ] Service logs startup message: "INFO: Application startup complete"
- [ ] FastAPI generates OpenAPI docs at `/docs`

**Health Check:**
- [ ] `curl http://localhost:3007/health` returns 200
- [ ] Response contains `{ "status": "healthy", "service": "analysis-service", "port": 3007 }`

**Document Analysis API:**
- [ ] `POST /analyze` accepts LegalDocument JSON
- [ ] Returns analysis metrics object
- [ ] Word count accurate (matches manual count)
- [ ] Sentence count accurate (matches manual count)
- [ ] Paragraph count accurate (matches manual count)
- [ ] Citation count matches `citations` array length
- [ ] Section count matches `body.sections` array length
- [ ] Average sentence length calculated correctly

**Readability Calculation:**
- [ ] Returns readability score (0-100)
- [ ] Returns readability level (very_easy, easy, moderate, difficult, very_difficult)
- [ ] Score correlates with average sentence length
- [ ] Longer sentences result in lower scores (more difficult)

**Section Analysis:**
- [ ] Returns analysis for each section
- [ ] Section analysis includes title, word_count, sentence_count, paragraph_count
- [ ] Section counts accurate per section

**Error Handling:**
- [ ] Returns 422 for invalid LegalDocument schema
- [ ] Returns 500 for analysis errors
...
- [ ] All 4 services start simultaneously without port conflicts
- [ ] Each service binds to its assigned port (3004, 3005, 3006, 3007)
- [ ] All health checks respond within 5 seconds of startup

**End-to-End Workflow:**
- [ ] Create template with variables → Template Service fills variables → Export as DOCX
- [ ] Parse citations from text → Citation Service validates → Link to document
- [ ] Upload evidence file → Evidence Service stores → Retrieve file → Delete file
- [ ] Draft document → Analysis Service analyzes → Returns metrics

**Service Discovery Readiness:**
- [ ] All services expose `/health` endpoint
- [ ] All services respond to health checks
- [ ] Services ready for Runbook 7 orchestrator integration

---

## Success Criteria

✅ All 4 services running on assigned ports
✅ All health checks responding
✅ All API endpoints functional
✅ Citation parsing accurate (case, statute, short forms)
✅ Evidence file upload/retrieval/deletion working
✅ Template variable replacement working
✅ Document analysis metrics accurate
✅ Cross-service integration tests passing
✅ No TypeScript compilation errors
✅ No Python runtime errors
✅ Service discovery ready
✅ Ready for Runbook 7 (Desktop Orchestrator)
